...
const cardinality = BigInt(
  Object.keys(palette).filter((a) => a.length === 1).length,
);
...
addEventListener("click", (event) => {
  const changeColor =
    event.target.dataset?.color ?? event.target.getAttribute("fill");
...
function encodeBoard() {
  ...
  codeInput.value =
    board[0].length.toString(16).padStart(2, "0") +
    board.length.toString(16).padStart(2, "0") +
    bigint.toString(16);
...
function update() {
  if (pointer.held) {
    ...
    const boardLeft = 0; // 0.5 * (SCREEN_WIDTH - width * cellSize)
    const boardTop = 0; // 0.5 * (SCREEN_HEIGHT - height * cellSize)
