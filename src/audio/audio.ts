...
export const initializeAudio =
  (startMusic: ExtendedBool) => (con: AudioContext) => {
...
export function step() {
  ...
  const duration = 0.2;
  const frequency = convertMidiToFrequency(
    stepNotes[randomUint32LessThan(prng, stepNotes.length)]!,
  );
