...
import { ShortBool } from "./shortbool.ts";
import { startMainloop } from "../node_modules/natlib/scheduling/mainloop.js";
import {
  enterPhase,
  interpolatePhase,
  updatePhase,
} from "../node_modules/natlib/state.js";
...
function updateControls() {
  ...
  if (pointer.held) {
    ...
    const iconsAreaWidth3 =
      3 * Settings.ICON_SIZE + 2.5 * Settings.ICON_SPACING; // .Inline(1)
    const iconsAreaWidth2 =
      2 * Settings.ICON_SIZE + 1.5 * Settings.ICON_SPACING; // .Inline(1)
    const iconsAreaWidth1 =
      1 * Settings.ICON_SIZE + 0.5 * Settings.ICON_SPACING; // .Inline(1)

    if (
      pointer.x >= Settings.SCREEN_WIDTH - iconsAreaWidth3 &&
      pointer.x < Settings.SCREEN_WIDTH &&
      pointer.y < Settings.ICON_SIZE
    ) {
    ...
    if (
      ...
      pointer.x <
        Settings.SCREEN_WIDTH - (level.boardLeft + 2 * level.cellSize) &&
      ...
      if (!duckState.pointerHeld) {
        ...
        window.location = Settings.COMMUNITY_LEVELS_URL;
      }
      return;
    }
  ...
  if (direction) {
    ...
    if (direction.x) {
      ...
      if (!level.tryMove(ducks[0]!, direction.x as MoveScalarNonzero, 0))
        return;
    }

    if (direction.y) {
      ducks.sort((a, b) => direction.y * (b.y - a.y));
      if (!level.tryMove(ducks[0]!, 0, direction.y as MoveScalarNonzero))
        return;
    }
  }
}
...
function render(t: number) {
  const tOscillator = interpolatePhase(
    oscillatorState,
    Settings.OSCILLATOR_DURATION,
    t,
  );
...
document.addEventListener(
  ...
  () => {
    ...
    audioHandle.initialize(
      initializeAudio(pointer.x < 0.5 * Settings.SCREEN_WIDTH),
    );
