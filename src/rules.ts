...
export function cascadeMove(
  ...
  if (
    board.positions[y]![x]!.some(
      (p) => collide.has(p.type) && (!p.cluster || p.cluster !== piece.cluster),
    )
  )
    // Collision, stop
    return;
  ...
  const cluster: Piece[] =
    piece.cluster && piece.cluster !== cause?.cluster
  ...
  const active: Piece[] = cluster.concat(
    board.positions[y]![x]!.filter(
      (p) => push.has(p.type) && (!p.cluster || p.cluster !== piece.cluster),
    ),
  );
