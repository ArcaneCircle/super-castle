...
export class Level {
  ...
  tryMove(piece: Piece, Δx: number, Δy: number): ExtendedBool {
    ...
    for ([piece, Δx, Δy] of plan) {
      ...
      if (
        this.board.positions[piece.y]![piece.x]!.some(
          (p) => p.type === PieceType.CUTTER,
        )
      ) {
  ...
  updateDucksOnGoal(collection: Set<Piece>) {
    this.board.pieces[PieceType.DUCK]?.forEach((duck) => {
      const onGoal = this.board.positions[duck.y]![duck.x]!.some(
        (p) => p.type === PieceType.GOAL,
      );
  ...
  connectDucklings(ducks: Piece[]) {
    ...
    ducks.forEach((duck) => {
      this.board
        .getBorderingPieces(duck, PieceType.DUCKLING)
        ?.forEach((duckling) => {
    ...
    clusters.forEach((cluster) => {
      cluster.pieces.forEach((duckling) => {
        ...
        const duck = this.board.createPiece(
          PieceType.DUCK,
          duckling.x,
          duckling.y,
        );
  ...
  splitCluster(cluster: Cluster) {
    ...
    for (const piece of pieces) {
      const group = [...this.board.getGroup(piece)].filter((p) =>
        pieces.has(p),
      );
    ...
    if (type === PieceType.DUCK && clusters.length > 1) {
      ...
      for (let n = 1; n < clusters.length; ++n) {
        ...
        clusters[n]!.pieces.forEach((duck) => {
          ...
          const duckling = this.board.createPiece(
            PieceType.DUCKLING,
            duck.x,
            duck.y,
          );
  ...
  checkWin(): ExtendedBool {
    ...
    if (
      duckCount === goalCount &&
      duckCount === this.ducksOnGoal.size &&
      duckCount === this.ducksOnGoalNext.size
    ) {
  ...
  render(t: number, tOscillator: number) {
    ...
    const duckColors = [
      ...
      colorDuckEntering,
      colorDuckLeaving,
      Palette.DUCK_ON_GOAL,
    ];
    const duckSecondaryColors = [
      Palette.DUCK_2,
      secondaryColorDuckEntering,
      secondaryColorDuckLeaving,
      Palette.DUCK_ON_GOAL_2,
    ];
    ...
    for (let y = 0; y < this.board.height; ++y) {
      for (let x = 0; x < this.board.width; ++x) {
        ...
        const tVibe = wrapAround(
          tOscillator + Settings.OSCILLATOR_INCREMENT * (x - 0.85 * y),
        );

        pieces.forEach(
          (piece) =>
            piece.type !== PieceType.GOAL &&
            this.renderPiece(
              piece,
              x,
              y,
    ...
    switch (piece.type) {
      case PieceType.DUCK:
        ...
        paintBlock(
          ...
          0,
          Settings.BLOCK_REFLECTION_OPACITY,
        );
...
function paintBlock(
  ...
  if (reflectionOpacity) {
    ...
    con.fillRect(
      x,
      y + size - Settings.BLOCK_GROW,
      size,
      height + Settings.BLOCK_GROW,
    );
  }
  ...
  con.fillRect(
    x + progress,
    y - height + size - Settings.BLOCK_GROW,
    size - progress,
    height + Settings.BLOCK_GROW,
  );
...
class LevelSelect extends Level {
  override checkWin(): ExtendedBool {
    ...
    let goal: Piece;
    if (
      this.board.positions[y]![x]!.some(
        (p) => p.type === PieceType.GOAL && (goal = p),
      )
    ) {
  ...
  override render(t: number, tOscillator: number) {
    ...
    printCenter(x, y, 4, "CLICK HERE TO IMPORT LEVEL", 1, tOscillator);
  ...
  override levelText() {
    ...
    for (let n = 1; n < levels.length - 2; ++n) {
      ...
      printCenter(
        x0,
        y0 + 0.5 * Settings.LEVEL_SELECT_FONT_SIZE,
        Settings.LEVEL_SELECT_FONT_SIZE,
        ch,
        0,
        0,
      );
...
function outline({
  ...
  boardLeft,
  boardTop,
}: Readonly<Level>): Path2D {
  const ch: Cell[][] = Array.from({ length: board.height + 1 }, (_, y) =>
  ...
  for (let y = 0; y < board.height; ++y) {
    for (let x = 0; x < board.width; ++x) {
      ...
      const up =
        y > 0 && ch[y - 1]![x]!.value ? null : new Edge(x, y, x + 1, y);
      const right = ch[y]![x + 1]!.value
        ? null
        : new Edge(x + 1, y, x + 1, y + 1);
      const down = ch[y + 1]![x]!.value
        ? null
        : new Edge(x + 1, y + 1, x, y + 1);
