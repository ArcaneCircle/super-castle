...
export function renderIntro(t: number, tOscillator: number) {
    ...
    con.beginPath()
    printCenter(0.5 * Settings.SCREEN_WIDTH, lerp(-20, 0.25 * Settings.SCREEN_HEIGHT, easeOutQuad(t0)), 6, 'SUPER CASTLE', 1.5, tOscillator)
    // con.lineWidth = 3
...
export function renderIntroEnd(t: number, tOscillator: number) {
    ...
    con.beginPath()
    printCenter(0.5 * Settings.SCREEN_WIDTH, 0.25 * Settings.SCREEN_HEIGHT, 6, 'SUPER CASTLE', 1.5, tOscillator)
    // con.lineWidth = 3
    // con.strokeStyle = Palette.NOTHING
    // con.stroke()
