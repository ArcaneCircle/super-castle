...
export function renderIntro(t: number, tOscillator: number) {
  const t0 = interpolatePhase(
    duckState,
    duckState.phase === DuckPhase.TITLE_SCREEN
      ? Settings.TITLE_ENTER_DURATION
      : Settings.LEAVE_DURATION,
    t,
  );
  ...
  con.fillRect(
    0,
    0.5 * (1 - t) * Settings.SCREEN_HEIGHT,
    Settings.SCREEN_WIDTH,
    t * Settings.SCREEN_HEIGHT,
  );
  ...
  printCenter(
    ...
    lerp(-20, 0.25 * Settings.SCREEN_HEIGHT, easeOutQuad(t0)),
    6,
    "SUPER CASTLE",
    1.5,
    tOscillator,
  ...
  if (duckState.phase === DuckPhase.TITLE_SCREEN) {
    ...
    printCenter(
      right,
      top - 0.05 * Settings.SCREEN_HEIGHT,
      6,
      "START",
      1,
      tOscillator,
...
export function renderIntroEnd(t: number, tOscillator: number) {
  ...
  con.beginPath();
  con.rect(
    0.5 * t * Settings.SCREEN_WIDTH,
    0,
    (1 - t) * Settings.SCREEN_WIDTH,
